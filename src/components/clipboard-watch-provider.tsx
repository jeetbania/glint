...
/**
 * Electron-only: watches the OS clipboard (via a main-process poller in
 * electron/main.js, relayed through electron/preload.js's
 * `window.glint.onClipboardChanged`) and offers to save new
 * screenshots/links/text even when the app isn't focused — not possible
 * from a plain web page, since browsers deliberately don't allow passive
 * background clipboard access for security reasons. Reuses the exact
 * same ingest path as in-app paste capture (useCaptureIngest), so a
 * clipboard-watch save and a Cmd+V paste behave identically.
 */
export function ClipboardWatchProvider() {
  ...
  useEffect(() => {
    ...
    const glint = (window as { glint?: { onClipboardChanged?: (cb: (payload: ClipboardCapture) => void) => () => void } }).glint;
    const unlisten = glint?.onClipboardChanged?.((payload) => {
